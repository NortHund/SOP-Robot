...
class EmotionDetection(Node):
    ...
    async def detect_emotion_callback(self, image_msg: Image):
        ...
        #with self.stop_time_lock:
        if self.start_time_bool == True:
            self.logger.info('timing e_n %s' % (time.time() - self.node_start_time) )
            self.start_time_bool = False
            #self.sending_emotion = False         
